...
public class ChessMatch {
    ...
    public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition){
        ...
        if (testCheck(currentPlayer)){
            ...
            throw new ChessException("You can´t puy yourself in check");
        }
    ...
    private boolean testCheck(Color color){
        ...
        for (Piece temp : opponentPieces){
            boolean[][] matrix = temp.possibleMoves();
    ...
    private boolean testCheckMate(Color color) {
        ...
        for (Piece temp : list) {
            ...
            for (int i = 0; i < board.getRows(); i++){
                for (int j = 0; j < board.getColumns(); j++){
                    if (matrix[i][j]){
                        Position source = ((ChessPiece)temp).getChessPosition().toPosition();
    ...
    public ChessPiece replacePromotedPiece(String type){
        ...
        Piece temp = board.removePiece(position);
        piecesOnTheBoard.remove(temp);
    ...
    private Piece makeMove(Position source, Position target){
        ChessPiece temp = (ChessPiece) board.removePiece(source);
        temp.increaseMoveCount();
        Piece capturedPiece = board.removePiece(target);
        board.placePiece(temp, target);
        ...
        if (temp instanceof King && target.getColumn() == source.getColumn() + 2){
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece){
        ChessPiece temp = (ChessPiece) board.removePiece(target);
        temp.decreaseMoveCount();
        board.placePiece(temp, source);
